...
export function AppProvider({ children }) {
  ...
  const [favoriteContacts, setFavoriteContacts] = useLocalStorage("kb_fav_contacts", ["Priya Patel", "Aarav Sharma"]);
  const [messages, setMessages] = useLocalStorage("kb_messages", INITIAL_MESSAGES);
  // Every message needs a stable id (not its array index) so delete/edit/star/
  // pin/react/vote keep pointing at the right message even after earlier
  // messages are removed. Seed data ships without ids, so backfill once.
  useEffect(() => {
    setMessages((m) => {
      let changed = false;
      const next = {};
      for (const chatName in m) {
        next[chatName] = m[chatName].map((msg, i) => {
          if (msg.id) return msg;
          changed = true;
          return { ...msg, id: `${chatName}__seed__${i}` };
        });
      }
  ...
  const deleteMessage = useCallback((chatName, msgId) => {
    setMessages((m) => ({ ...m, [chatName]: (m[chatName] || []).filter((x) => x.id !== msgId) }));
  }, [setMessages]);

  const editMessage = useCallback((chatName, msgId, text) => {
    setMessages((m) => ({ ...m, [chatName]: (m[chatName] || []).map((x) => (x.id === msgId ? { ...x, text, edited: true } : x)) }));
  }, [setMessages]);

  const toggleStar = useCallback((chatName, msgId) => {
    setMessages((m) => {
      ...
      return { ...m, [chatName]: list.map((x) => (x.id === msgId ? { ...x, starred: willStar } : x)) };
    });
    setTimeout(checkAchievements, 200);
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [setMessages, setStarred]);

  const togglePinMessage = useCallback((chatName, msgId) => {
    setMessages((m) => ({ ...m, [chatName]: (m[chatName] || []).map((x) => (x.id === msgId ? { ...x, pinned: !x.pinned } : x)) }));
  }, [setMessages]);

  const reactToMessage = useCallback((chatName, msgId, emoji) => {
    setMessages((m) => ({
      ...m,
      [chatName]: (m[chatName] || []).map((x) => (x.id === msgId ? { ...x, reactions: [...(x.reactions || []), emoji] } : x))
    }));
    playSound("react");
    haptic(20);
  }, [setMessages, playSound, haptic]);

  const votePoll = useCallback((chatName, msgId, optIdx) => {
    setMessages((m) => ({
      ...m,
      [chatName]: (m[chatName] || []).map((x) => {
        if (x.id !== msgId || !x.poll) return x;
        if (x.poll.voted) return x; // already voted on this poll — ignore repeat taps
        const options = x.poll.options.map((o, oi) => (oi === optIdx ? { ...o, v: o.v + 1 } : o));
        return { ...x, poll: { ...x.poll, options, voted: true, votedOption: optIdx } };
      })
    }));
    playSound("react");
  ...
  const toggleFavoriteContact = useCallback((name) => {
    setFavoriteContacts((f) => (f.includes(name) ? f.filter((n) => n !== name) : [...f, name]));
  }, [setFavoriteContacts]);
  ...
  const value = useMemo(() => ({
    ...
    setChatMeta, createGroup, addGroupMembers, removeGroupMember, renameGroup, deleteChat, favoriteContacts, toggleFavoriteContact,
